...
namespace ITAssetManagerComponents.Services
{
    ...
    public class ExcelDataService : IExcelDataService
    {
        ...
        public async Task<int> ImportFromExcelAsync<T>(Stream excelStream, string entityType) where T : class, new()
        {
            ...
            // 활동 시트명 결정
            string rcKey, svKey, mKey, fKey;
            if (entityType == IExcelDataService.SERVER_KEY)
            {
                rcKey = IExcelDataService.SERVER_ROUTINE_CHECKS_KEY;
                svKey = IExcelDataService.SERVER_SECURITY_VULNERABILITIES_KEY;
                mKey = IExcelDataService.SERVER_MAINTENANCES_KEY;
                fKey = IExcelDataService.SERVER_FAILURES_KEY;
            }
            else
            {
                rcKey = "RoutineChecks";
                svKey = "SecurityVulnerabilities";
                mKey = "Maintenances";
                fKey = "Failures";
            }
            ...
            var routineChecks = ReadSheet<RoutineCheck>(rcKey);
            var securityVulns = ReadSheet<SecurityVulnerability>(svKey);
            var maintenances = ReadSheet<Maintenance>(mKey);
            var failures = ReadSheet<Failure>(fKey);
